...
_TAR_FILE = 'dist-awsome.tar.gz'
_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE
_REMOTE_BASE_DIR = '/srv/awsome'
...
def deploy():
	...
	with settings(warn_only = True):
		sudo('supervisorctl stop awsome')
...
def rollback():
	...
	with cd(_REMOTE_BASE_DIR):
		...
		if len(ss) != 2:
			...
			return
		current = ss[1]
		print('Found currend symbol link points to: %s\n' % current)
		try:
		...
		for f in files:
			if f == current:
				...
				print('		Rollvack to ---> %s ' % old)
			else:
				print('			%s' % f)
		print('=========================================')
		print('')
		yn = raw_input('contine? y/n')
		if yn != 'y' and yn != 'Y':
